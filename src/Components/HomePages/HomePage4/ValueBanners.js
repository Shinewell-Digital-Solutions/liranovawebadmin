...
const ValueBanners = ({ values, setFieldValue, productData, categoryData }) => {
    ...
    return (
        <>
            ...
            {values['content']?.['value_banners']?.['banners'] && <Btn className="btn-theme my-4" onClick={() => setFieldValue("[content][value_banners][banners]", [...values['content']['value_banners']['banners'], { title: "", description: "" }])} title="add_banners" />}
            {
