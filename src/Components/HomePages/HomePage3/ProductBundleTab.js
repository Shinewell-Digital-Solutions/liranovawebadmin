...
const ProductBundleTab = ({ values, setFieldValue }) => {
    ...
    return (
        <>
            ...
            {values['content']?.['product_bundles']?.['bundles'] && values['content']?.['product_bundles']?.['bundles'].length < 4 && <Btn className="btn-theme my-4" onClick={() => setFieldValue("[content][product_bundles][bundles]", [...values['content']?.['product_bundles']?.['bundles'], { title: "", description: "" }])} title="add_banners" />}
            {
